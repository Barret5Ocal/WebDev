...
/*
*Barret's Debug*
The code is setting offset to -1 when there is no match 
I'm not sure if the code actually knows how to stop when the match fails 

Ok, it does not infinite loop anymore
but now it does not actually match
...
#define GBRE__LITERAL(str) (str), gbre_size_of(str)-1
    ...
    static gbreContext gbre__consume(gbRegex *re, isize op, char const *str, isize str_len, isize offset,
                                     gbreCapture *captures, isize max_capture_count,
                                     gbreBool is_greedy) { // NOTE(Barret5Ocal): I think I am getting caught in this 
        // NOTE(Barret5Ocal): What does this function do?
        ...
        if(offset != -1)
        {
            switch (re->buf[op++]) {
                ...
                case GBRE_OP_ANY_OF: {
                    isize i;
                    char cin = str[offset];
                    buffer_len = re->buf[op++]; // NOTE(Barret5Ocal): this gets the amount of stuff in the [] 
                    
                    if (offset >= str_len)
                        return gbre__context_no_match(op + buffer_len);
                    
                    for (i = 0; i < buffer_len; i++) {
                        char cmatch = (char)re->buf[op+i]; // NOTE(Barret5Ocal): get the zero if its an escape character, gets a match if its a match. This is also why I can look at the entire string in the debuger
                        if (!cmatch) {
                            i++;
                            if (gbre__match_escape(cin, re->buf[op+i] << 8))
                                break;
                        } else if (cin == cmatch) { // NOTE(Barret5Ocal): This is where the problem starts, the [A-Za-z0-9_] syntax does not work here(ranges). It is trying to match the character in the pattern with the actual characters in the [], either add in the functionality or change the regex. 
                            // NOTE(Barret5Ocal): but shouldn't this cause the whole match function to fail. 
                            break;
                        }
                    }
                    
                    if (i == buffer_len)
                        return gbre__context_no_match(op + buffer_len); // When the loop fails to match, it goes here and returns without a match. this is where the -1 comes from. I think i found the problem. -1 is the code for GBRE__NO_MATCH. when this returns with -1 offset, the whole fuction should fail, right?
                ...
                case GBRE_OP_ONE_OR_MORE: { // NOTE(Barret5Ocal): calls this function recurcively
    ...
    static gbreContext gbre__exec(gbRegex *re, isize op, char const *str, isize str_len, isize offset,
                                  gbreCapture *captures, isize max_capture_count) {
        // NOTE(Barret5Ocal): this might be the place to solve the problem. if we always exit apon -1 we should at least not loop infinitely 
        // NOTE(Barret5Ocal): there are acturally other places we have to do this
        gbreContext c;
        c.op = op;
        c.offset = offset;
        while (c.op < re->buf_len && c.offset != -1) {
    ...
    gbreBool gbre_match(gbRegex *re, char const *str, isize len, gbreCapture *captures, isize max_capture_count) {
        if (re && re->buf_len > 0) {
            if (re->buf[0] == GBRE_OP_BEGINNING_OF_LINE) {
                ...
                if (c.offset == GBRE__INTERNAL_FAILURE || c.offset == GBRE__NO_MATCH) return GBRE_FALSE;
            } else {
                isize i;
                for (i = 0; i < len; i++) {
